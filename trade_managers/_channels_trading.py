...
def channel_mid_trading(ticker, inner_safety_margin_ratio=0.15, outer_safety_margin_ratio=0.15):
    ...
    df_backup = pd.read_csv(download_stock_day(ticker))[-252:].reset_index()
    backcandles = 100
    ...
    trades_df.to_csv(save_under_results_path(f'{ticker}_channel_midline_trading_trades.csv'))
    trades_df.drop_duplicates(subset=['enter_candle_id'], inplace=True)
    trades_df.to_csv(save_under_results_path(f'{ticker}_channel_midline_trading_trades_dropped_duplicates.csv'))
...
if __name__ == '__main__':
    ...
    # trades_csv = r"C:\Users\Avishay Wasse\PycharmProjects\stock_market_analysis\results\AA_channel_midline_trading_trades_dropped_duplicates.csv"
    #
    # df = pd.read_csv(trades_csv)
    # print(len(df))
    # losses_df = df.loc[~df['win']].copy()
    # print('losses', len(losses_df))
    # losses_df.drop_duplicates(subset=['channel_candle_id'], inplace=True)
    # print('losses after dropping channel duplicates', len(losses_df))
    #
    # win_rate = len(df.loc[df['win']])/len(df)
    # avg_win = df.loc[df['win']]['change%'].mean()
    # avg_loss = df.loc[~df['win']]['change%'].mean()
    # print('win rate', win_rate)
    # print('avg win', avg_win)
    # print('avg loss', avg_loss)
    # print('cumulative change',
    #       np.power(1 + (avg_win / 100.0), win_rate * 10.0) * np.power(1 + (avg_loss / 100.0), (1 - win_rate) * 10.0))
    # df.drop_duplicates(subset=['enter_candle_id'], inplace=True)
    ...
    # print('cumulative change',
    #       np.power(1 + (avg_win / 100.0), win_rate * 10.0) * np.power(1 + (avg_loss / 100.0), (1 - win_rate) * 10.0))
    # df.to_csv(save_under_results_path('AA_channel_midline_trading_trades_dropped_duplicates.csv'))
